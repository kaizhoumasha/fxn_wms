...
### Session 2025-12-12
- Q: 设备指令下发到执行完成的 p95 目标设定？ → A: p95 ≤10s（设备指令下发到执行完成）
...
- **FR-004**: 给出性能/容量设计基线（设计目标值）与观测方案：指标（Metrics）、日志（Logs）、追踪（Traces）口径，含采集位置与验收方法；设备指令下发到执行完成的链路延迟目标为 p95 ≤10s。  
...
- **SC-004**: 观测性方案覆盖所有关键节点，定义的指标/日志字段可直接转化为验收脚本；设备指令下发到执行完成 p95 ≤10s 的设计目标写明度量口径与采集点。  
