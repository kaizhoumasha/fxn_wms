...
| 功能模块     | WMS | WES | ECS | 说明                          |
| ------------ | --- | --- | --- | ----------------------------- |
| 装箱任务呼叫 | ✅  | -   | -   | WMS 提供 Web 界面和 PDA 界面  |
| 空架监控     | -   | ✅  | -   | WES 实时监控装箱区空架数量    |
| 空架补给请求 | ✅  | ✅  | -   | WES 发起请求，WMS 调度 RCS    |
| 空架扫描验证 | -   | ✅  | ✅  | ECS 扫描，WES 验证            |
| 栈板拆箱     | ✅  | -   | -   | WMS 提供 PDA 拆栈板界面       |
| PKG 视觉扫描 | -   | -   | ✅  | ECS 视觉系统扫描 PKG 六合一码 |
| PKG 校验     | -   | ✅  | -   | WES 验证 PKG 是否属于当前 GRN |
| 智能分箱算法 | -   | ✅  | -   | WES 执行分箱算法              |
| 放入指令下发 | -   | ✅  | -   | WES 下发指令给 ECS            |
| 物理放入动作 | -   | -   | ✅  | ECS 机械臂执行放入            |
| PKG 绑定通知 | ✅  | ✅  | -   | WES 通知 WMS 更新库存         |
| 满架检测     | -   | ✅  | -   | WES 计算货架容量              |
| 满架搬运请求 | ✅  | ✅  | -   | WES 发起请求，WMS 调度 RCS    |
| RCS 调度     | ✅  | -   | -   | WMS 调度 RCS 执行搬运任务     |
| 异常处理     | ✅  | ✅  | ✅  | 三方协同处理异常              |
...
```json
// 启动间隔步进模式
POST /ecs/api/conveyor/start-interval-loading
{
  "task_id": "TASK-001",
  "line_id": "LINE_1",
  "rods_needed": 2  // 需要处理的串杆数量
}

// 响应
{
  "result": "SUCCESS",
  "message": "Interval loading started"
}
```
...
- **ECS 负责**: 硬件控制、步进节奏、传感器监控、自动触发
- **WES 负责**: 启动/停止指令、接收完成通知
- **WMS 负责**: 计算上料参数 (rods_needed)
...
3. **启动间隔上料** (自动):
   ...
   - WES → ECS: 启动间隔上料模式
     ```json
     {
       "command": "START_INTERVAL_LOADING",
       "task_id": "TASK-001",
       "line_id": "LINE_1"
     }
     ```
...
- **WES 负责**: 启动/停止指令下发、接收完成通知
- **ECS 负责**: 串杆步进控制、上料节奏管理、视觉扫描触发
- **人工负责**: 拆箱、将料盘放入串杆
...
**业务场景**: WES 检测货架已满，自动向 WMS 提交搬运任务和空架补给请求。
...
1. **满架检测** (WES):
   ...
   - WES 定时检查货架容量（间隔 10 秒）
   - 检查条件: `used_capacity / total_capacity >= 0.95`
   - 或无可用储位分配新料盘
2. **标记货架为满** (WES):
...
sequenceDiagram
    ...
    WMS-->>作业员: 显示栈板信息
    作业员->>WMS: PDA 扫描箱条码
    WMS->>WMS: 计算上料参数 (rods_needed)
    WMS-->>作业员: 显示箱信息和上料提示
    WMS->>WES: 通知箱扫描完成 (rods_needed)
    WES->>ECS: 启动间隔步进模式
    ECS->>ECS: 串杆 1 步进到 Position 0 (上料位)
    ECS->>ECS: 启动硬件倒计时（30 秒）
    作业员->>作业员: 拆箱，将料盘放入串杆 1
...
```json
{
  ...
  "dims": "7inch",
  "thickness": 12.5,
  "location": "KITTING-RACK-001-BIN-001-SLOT-01",
  "status": "IN_RACK",
  "grn_id": "GRN.0001",
  "pallet_id": "PAL.0001",
  "bound_at": "2025-12-22T10:30:00Z"
}
```

#### 3. 装箱任务数据 (Kitting Task Data)
...
```json
{
  ...
  "started_at": "2025-12-22T09:30:00Z",
  "estimated_completion": "2025-12-22T11:00:00Z"
}
```

#### 4. 执行状态数据 (Execution State Data)
...
```json
{
  "execution_id": "EXEC-20251222-001",
  "pkg_code": "CAP001-5000-PKG12345-LC01-DC02-V0001",
  "current_stage": "BINNING",
  "rack_id": "RACK-001",
  "bin_id": "BIN-001",
  "slot_id": "SLOT-01",
  "expected_height": 37.5,
...
1. **最终一致性**: WES 货架状态与 WMS 库存数据最终一致
2. **实时同步**: PKG 绑定后，WES 立即通知 WMS 更新库存
3. **定时校验**: WES 每小时与 WMS 校验一次库存数据
