...
| **任务生成**       | ✅ 生成发料任务          | -                | -               | -                  | -                |
| **货架规划**       | ✅ 货架预规划与物料定位  | -                | -               | -                  | -                |
| **货架调度**       | ✅ 调度 RCS 搬运货架     | -                | -               | -                  | ✅ 执行货架搬运  |
| **任务下发**       | ✅ 下发任务给 WES        | ✅ 接收任务      | -               | -                  | -                |
| **料箱批量上料**   | -                        | ✅ 协调 CTU 上料 | -               | ✅ 批量取箱放入流水线 | -                |
| **料箱识别**       | -                        | ✅ 决策是否进入工作位 | ✅ 扫描料箱码   | -                  | -                |
| **自动化执行**     | -                        | ✅ 协调 ECS 执行 | ✅ 执行拣料动作 | -                  | -                |
| **硬件交互**       | -                        | ✅ 下发拣料指令  | ✅ 上报执行结果 | -                  | -                |
| **料箱回库**       | ✅ 调度 CTU 回库         | ✅ 请求 CTU 回库 | -               | ✅ 将料箱放回货架  | -                |
| **结果上报**       | ✅ 接收完成结果          | ✅ 上报完成结果  | -               | -                  | -                |
| **库存更新**       | ✅ 更新库存数据          | -                | -               | -                  | -                |
...
1. **CTU 上料完成信号**: CTU 上料箱到流水线后,需要 WES 发送 `PUT_FINISH` 命令到 ECS,通知流水线 CTU 已完成放料动作,流水线才能继续后续操作
2. **CTU 下料完成信号**: CTU 下料箱时,需要 WES 发送 `PICK_FINISH` 命令到 ECS,通知流水线料箱已被完全取走,流水线才能确定可以继续后续动作
...
2. **五层货架批量上料** (CTU → 流水线) - **效率优化关键**
   ...
   - **操作步骤**:
     ...
     - **WES 发送 `PUT_FINISH` 信号**: CTU 每次放料完成后，WES 需发送 `PUT_FINISH` 命令到 ECS，通知流水线 CTU 已完成放料动作
     - 流水线收到信号后自主启动，将料箱送往工作位
...
8. **出料口回库** (ECS → WES → WMS)
   ...
   - **执行**: WMS 调度 RCS/CTU 将料箱从出料口取走
   - **WES 发送 `PICK_FINISH` 信号**: CTU 取走料箱后，WES 需发送 `PICK_FINISH` 命令到 ECS，通知流水线料箱已被完全取走
   - **回库**: CTU 将料箱放回五层货架
...
sequenceDiagram
    ...
    and 五层货架流程 (CTU + 流水线)
        ...
        loop CTU 逐个放箱
            RCS->>ECS: CTU 将料箱放入流水线入料口
            WES->>ECS: POST /api/v1/device/command (PUT_FINISH)
            Note right of WES: 通知流水线 CTU 已完成放料
            ECS-->>WES: 200 OK
            ECS->>ECS: 流水线启动，料箱送往工作位
        end
        ...
        and 出料口 CTU 回库 (异步执行)
            loop CTU 回库作业
                WMS->>RCS: 调度 CTU 执行回库
                RCS->>RCS: CTU 从出料口取走料箱
                WES->>ECS: POST /api/v1/device/command (PICK_FINISH)
