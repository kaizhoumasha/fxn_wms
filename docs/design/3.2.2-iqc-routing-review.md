...
- **PalletGrnBindings** (栈板-GRN 绑定关系): `binding_id`, `pallet_id`, `grn_number`, `material_code`, `tray_count`, `status`(PENDING/IN_PROGRESS/COMPLETED), `created_at`, `updated_at`。**说明**: 支持一个栈板包含多个 GRN（混托），一个 GRN 分布在多个栈板。通过此表查询栈板关联的所有 GRN 信息。
- **Sampling_Record** (取样记录): `sampling_id`, `pallet_id`, `grn_number`, `pkg_code`, `material_id`, `quantity`, `sampled_by`, `sampled_at`, `returned_at`, `returned_by`, `status`(SAMPLED/RETURNED)。
- **Transport_Task** (搬运任务): `task_id`, `pallet_id`, `from_location`, `to_location`, `priority`, `status`, `created_at`, `completed_at`, `rcs_ref`。
- **Inspection_Result** (检验结果): `grn_number`, `result`(OK/NG), `inspected_by`, `inspected_at`, `remarks`(由 QMS 提供,WMS 查询)。
- **Review_Result** (复判结果): `grn_number`, `result`(OK/NG/SORTING), `reviewed_by`, `reviewed_at`, `remarks`(由 QMS 提供,WMS 查询)。
- **Split_Record** (拆板记录): `split_id`, `original_pallet_id`, `new_pallet_id`, `material_id`, `split_qty`, `split_by`, `split_at`, `reason`。
- **Priority_Adjustment_Log** (优先级调整日志): `log_id`, `task_id`, `old_priority`, `new_priority`, `adjusted_by`, `adjusted_at`, `reason`。
