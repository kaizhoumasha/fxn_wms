...
6. **WES**: 查询并筛选可用货架资源 (Resource Discovery)
   ```python
   # 1. 筛选状态正常的五层货架 (Online & Healthy)
   # 2. 筛选具有符合类型 (A/B) 空箱的货架
   candidate_racks = query_available_resources(target_area="SMT_STORAGE", bin_type=source_bin.type)
   ```
...
7. **WES**: 执行最优目标货架选择算法 (Target Selection Algorithm)
   ...
   def select_optimal_target(candidates, material_info):
       ...
       best_bin = max(candidates, key=lambda x: calculate_score(x, material_info))
       return best_bin
   ```
