...
> **作者**: WMS/WES 联合设计团队
> **修订记录**:
> - v1.1 (2025-12-25): 补充数据库模型 - 新增 inbound_strategy_decision、exchange_task、picking_task、phase_execution_log、empty_bin_allocation 表，完善决策审计、任务追踪、阶段管理和资源分配功能
...
### 8.4 持久化数据库模型 (Hybrid Inbound Schema)

```sql
-- 混合入库任务表 (Hybrid Inbound Task)
-- 描述: 承载满箱交换/优先交换/零散入库三种模式的顶层任务
CREATE TABLE hybrid_inbound_task (
  task_id VARCHAR(50) PRIMARY KEY COMMENT '任务 ID (与 UI/监控一致)',
  inbound_mode ENUM('FULL_EXCHANGE', 'PRIORITY_EXCHANGE', 'PIPELINE_PICKING') NOT NULL COMMENT '入库模式',
  source_rack_id VARCHAR(50) NOT NULL COMMENT '单层货架 (来自 3.3.1)',
  target_rack_id VARCHAR(50) NOT NULL COMMENT '五层货架 (目标)',
  decision_payload JSON NULL COMMENT 'WES 决策结果原文，用于审计/重放',
  transport_task_id VARCHAR(50) NULL COMMENT '关联搬运任务 (transport_task.task_id)',
  status ENUM('PENDING', 'DISPATCHED', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'CANCELLED') DEFAULT 'PENDING',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (source_rack_id) REFERENCES Racks(rack_id),
  FOREIGN KEY (target_rack_id) REFERENCES Racks(rack_id)
) COMMENT='Hybrid 入库顶层任务';

-- 料箱交换/拣选子任务表 (Hybrid Sub Task)
-- 描述: 细化到单个料箱/储位的原子操作，便于并行跟踪
CREATE TABLE hybrid_sub_task (
  sub_task_id VARCHAR(50) PRIMARY KEY,
  task_id VARCHAR(50) NOT NULL COMMENT '归属顶层任务',
  sub_type ENUM('BIN_EXCHANGE', 'BIN_PICKING') NOT NULL COMMENT '子任务类型',
  source_bin_id VARCHAR(50) NOT NULL COMMENT '来源料箱',
  target_bin_id VARCHAR(50) NULL COMMENT '目标料箱 (交换/拣选目标)',
  target_slot_id VARCHAR(50) NULL COMMENT '目标储位 (零散入库)',
  priority INT DEFAULT 5 COMMENT '优先级 (1-10)',
  status ENUM('PENDING', 'DISPATCHED', 'IN_PROGRESS', 'COMPLETED', 'FAILED') DEFAULT 'PENDING',
  rcs_ref VARCHAR(100) NULL COMMENT 'RCS/CTU 外部任务引用',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (task_id) REFERENCES hybrid_inbound_task(task_id),
  FOREIGN KEY (source_bin_id) REFERENCES Bins(bin_id),
  FOREIGN KEY (target_bin_id) REFERENCES Bins(bin_id),
  FOREIGN KEY (target_slot_id) REFERENCES Slots(slot_id)
) COMMENT='Hybrid 入库子任务';

-- 混合入库告警/异常日志 (Hybrid Alert Log)
CREATE TABLE hybrid_alert_log (
  alert_id VARCHAR(50) PRIMARY KEY,
  task_id VARCHAR(50) NOT NULL,
  alert_type ENUM('CTU_ERROR', 'ECS_PICK_FAIL', 'LOCATION_CONFLICT', 'PIPELINE_BLOCK') NOT NULL,
  severity ENUM('INFO', 'WARNING', 'CRITICAL') DEFAULT 'WARNING',
  message TEXT COMMENT '描述/堆栈',
...
-- 入库策略决策表 (Inbound Strategy Decision)
-- 描述: 记录 WES 决策引擎的模式选择过程，用于审计和优化
CREATE TABLE inbound_strategy_decision (
  decision_id VARCHAR(50) PRIMARY KEY COMMENT '决策记录 ID',
  task_id VARCHAR(50) NOT NULL COMMENT '关联顶层任务',
  source_rack_id VARCHAR(50) NOT NULL COMMENT '单层货架 ID',
  bin_id VARCHAR(50) NOT NULL COMMENT '料箱 ID',

  -- 决策输入 (Decision Inputs)
  bin_usage_rate DECIMAL(5,2) NOT NULL COMMENT '料箱使用率 (%)',
  available_empty_bins INT NOT NULL COMMENT '可用空箱数量',
  material_code VARCHAR(50) COMMENT '物料代码',
  vendor_code VARCHAR(50) COMMENT '供应商代码',
  dc_code VARCHAR(50) COMMENT 'Date Code',

  -- 决策输出 (Decision Output)
  selected_mode ENUM('FULL_EXCHANGE', 'PRIORITY_EXCHANGE', 'PIPELINE_PICKING') NOT NULL COMMENT '选择的入库模式',
  decision_reason TEXT COMMENT '决策原因 (如: Usage=85% >= 80%, 有空箱 → 满箱交换)',
  target_bin_id VARCHAR(50) NULL COMMENT '目标料箱 ID (交换模式)',
  target_slot_id VARCHAR(50) NULL COMMENT '目标储位 ID (拣选模式)',

  -- 决策元数据 (Decision Metadata)
  decision_timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '决策时间',
  decision_duration_ms INT COMMENT '决策耗时 (毫秒)',
  config_snapshot JSON COMMENT '决策时的配置参数快照 (FULL_EXCHANGE_THRESHOLD等)',
  ...
  FOREIGN KEY (target_bin_id) REFERENCES Bins(bin_id),
  FOREIGN KEY (target_slot_id) REFERENCES Slots(slot_id),
  INDEX idx_decision_mode (selected_mode),
  INDEX idx_decision_timestamp (decision_timestamp)
) COMMENT='入库策略决策审计表';

-- 满箱交换任务表 (Exchange Task)
-- 描述: 专门追踪 CTU 执行的料箱交换操作，独立于通用 transport_task
CREATE TABLE exchange_task (
  exchange_task_id VARCHAR(50) PRIMARY KEY COMMENT '交换任务 ID',
  task_id VARCHAR(50) NOT NULL COMMENT '关联顶层任务',
  decision_id VARCHAR(50) NOT NULL COMMENT '关联决策记录',

  -- 交换源和目标 (Exchange Source & Target)
  source_rack_id VARCHAR(50) NOT NULL COMMENT '单层货架 ID',
  source_bin_id VARCHAR(50) NOT NULL COMMENT '源料箱 ID (单层货架上的满箱)',
  target_rack_id VARCHAR(50) NOT NULL COMMENT '五层货架 ID',
  target_bin_id VARCHAR(50) NOT NULL COMMENT '目标料箱 ID (五层货架上的空箱)',
  target_layer INT NOT NULL COMMENT '目标层号 (1-5)',
  target_column INT NOT NULL COMMENT '目标列号 (1-4)',
  target_side ENUM('A', 'B') NOT NULL COMMENT '目标面 (A/B)',

  -- 交换执行状态 (Exchange Execution Status)
  status ENUM('PENDING', 'RACK_MOVING', 'CTU_EXECUTING', 'COMPLETED', 'FAILED', 'CANCELLED') DEFAULT 'PENDING',
  ctu_command_id VARCHAR(100) COMMENT 'CTU 设备命令 ID',
  ctu_response JSON COMMENT 'CTU 响应数据',

  -- 时间追踪 (Time Tracking)
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  rack_arrived_at TIMESTAMP NULL COMMENT '货架到达交换区时间',
  ctu_started_at TIMESTAMP NULL COMMENT 'CTU 开始执行时间',
  ctu_completed_at TIMESTAMP NULL COMMENT 'CTU 完成时间',
  ...
  FOREIGN KEY (task_id) REFERENCES hybrid_inbound_task(task_id),
  FOREIGN KEY (decision_id) REFERENCES inbound_strategy_decision(decision_id),
  FOREIGN KEY (source_rack_id) REFERENCES Racks(rack_id),
  FOREIGN KEY (source_bin_id) REFERENCES Bins(bin_id),
  FOREIGN KEY (target_rack_id) REFERENCES Racks(rack_id),
  FOREIGN KEY (target_bin_id) REFERENCES Bins(bin_id),
...
-- 零散入库拣选任务表 (Picking Task)
-- 描述: 专门追踪 ECS 执行的料盘级拣选操作，包含 Pick_List 详情
CREATE TABLE picking_task (
  picking_task_id VARCHAR(50) PRIMARY KEY COMMENT '拣选任务 ID',
  task_id VARCHAR(50) NOT NULL COMMENT '关联顶层任务',
  decision_id VARCHAR(50) NOT NULL COMMENT '关联决策记录',

  -- 拣选源和目标 (Picking Source & Target)
  source_rack_id VARCHAR(50) NOT NULL COMMENT '单层货架 ID',
  source_bin_id VARCHAR(50) NOT NULL COMMENT '源料箱 ID',
  source_slot_id VARCHAR(50) NOT NULL COMMENT '源储位 ID',
  target_rack_id VARCHAR(50) NOT NULL COMMENT '五层货架 ID',
  target_bin_id VARCHAR(50) NOT NULL COMMENT '目标料箱 ID',
  target_slot_id VARCHAR(50) NOT NULL COMMENT '目标储位 ID',

  -- Pick List 详情 (Pick List Details)
  pick_list JSON NOT NULL COMMENT 'ECS 拣选清单 (PKG 列表, 数量, 物料信息)',
  material_code VARCHAR(50) COMMENT '物料代码',
  vendor_code VARCHAR(50) COMMENT '供应商代码',
  dc_code VARCHAR(50) COMMENT 'Date Code',
  planned_qty INT NOT NULL COMMENT '计划拣选数量 (料盘数)',
  actual_qty INT DEFAULT 0 COMMENT '实际拣选数量',

  -- 拣选执行状态 (Picking Execution Status)
  status ENUM('PENDING', 'RACK_MOVING', 'ECS_SCANNING', 'ECS_PICKING', 'COMPLETED', 'FAILED', 'CANCELLED') DEFAULT 'PENDING',
  ecs_command_id VARCHAR(100) COMMENT 'ECS 设备命令 ID',
  ecs_response JSON COMMENT 'ECS 响应数据 (扫描结果, 拣选结果)',

  -- 时间追踪 (Time Tracking)
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  rack_arrived_at TIMESTAMP NULL COMMENT '货架到达拣选区时间',
  ecs_scan_started_at TIMESTAMP NULL COMMENT 'ECS 扫描开始时间',
  ecs_pick_started_at TIMESTAMP NULL COMMENT 'ECS 拣选开始时间',
  ecs_completed_at TIMESTAMP NULL COMMENT 'ECS 完成时间',
  finished_at TIMESTAMP NULL,
  ...
  FOREIGN KEY (source_rack_id) REFERENCES Racks(rack_id),
  FOREIGN KEY (source_bin_id) REFERENCES Bins(bin_id),
  FOREIGN KEY (source_slot_id) REFERENCES Slots(slot_id),
  FOREIGN KEY (target_rack_id) REFERENCES Racks(rack_id),
  FOREIGN KEY (target_bin_id) REFERENCES Bins(bin_id),
  FOREIGN KEY (target_slot_id) REFERENCES Slots(slot_id),
  INDEX idx_picking_status (status),
  INDEX idx_picking_created (created_at)
) COMMENT='零散入库拣选任务表';

-- 阶段执行日志表 (Phase Execution Log)
-- 描述: 追踪顺序阶段执行 (决策 → 满箱交换 → 零散入库 → 回库)，支持断点恢复
CREATE TABLE phase_execution_log (
  log_id VARCHAR(50) PRIMARY KEY COMMENT '日志 ID',
  task_id VARCHAR(50) NOT NULL COMMENT '关联顶层任务',

  -- 阶段信息 (Phase Information)
  phase_type ENUM('DECISION', 'EXCHANGE', 'PICKING', 'RETURN') NOT NULL COMMENT '阶段类型',
  phase_sequence INT NOT NULL COMMENT '阶段序号 (1, 2, 3, 4)',
  phase_status ENUM('PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'SKIPPED') NOT NULL COMMENT '阶段状态',

  -- 阶段执行详情 (Phase Execution Details)
  started_at TIMESTAMP NULL COMMENT '阶段开始时间',
  completed_at TIMESTAMP NULL COMMENT '阶段完成时间',
  duration_seconds INT COMMENT '阶段耗时 (秒)',

  -- 阶段输入输出 (Phase Input/Output)
  input_data JSON COMMENT '阶段输入数据',
  output_data JSON COMMENT '阶段输出数据',

  -- 阶段检查点 (Phase Checkpoint)
  checkpoint_data JSON COMMENT '检查点数据 (用于断点恢复)',
  can_resume BOOLEAN DEFAULT TRUE COMMENT '是否可恢复',
  ...
  FOREIGN KEY (task_id) REFERENCES hybrid_inbound_task(task_id),
  INDEX idx_phase_task (task_id, phase_sequence),
  INDEX idx_phase_status (phase_status)
) COMMENT='阶段执行日志表';

-- 空箱分配记录表 (Empty Bin Allocation)
-- 描述: 追踪满箱交换时分配的空箱资源，防止并发冲突
CREATE TABLE empty_bin_allocation (
  allocation_id VARCHAR(50) PRIMARY KEY COMMENT '分配记录 ID',
  task_id VARCHAR(50) NOT NULL COMMENT '关联顶层任务',
  exchange_task_id VARCHAR(50) NULL COMMENT '关联交换任务',

  -- 空箱信息 (Empty Bin Information)
  rack_id VARCHAR(50) NOT NULL COMMENT '五层货架 ID',
  bin_id VARCHAR(50) NOT NULL COMMENT '空料箱 ID',
  layer INT NOT NULL COMMENT '层号 (1-5)',
  column_num INT NOT NULL COMMENT '列号 (1-4)',
  side ENUM('A', 'B') NOT NULL COMMENT '面 (A/B)',

  -- 分配状态 (Allocation Status)
  status ENUM('ALLOCATED', 'IN_USE', 'RELEASED', 'EXPIRED') DEFAULT 'ALLOCATED',
  allocated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '分配时间',
  locked_until TIMESTAMP NOT NULL COMMENT '锁定截止时间',
  released_at TIMESTAMP NULL COMMENT '释放时间',

  -- 分配原因 (Allocation Reason)
  allocation_reason TEXT COMMENT '分配原因 (如: 满箱交换, Usage=85%)',
  source_bin_id VARCHAR(50) COMMENT '源料箱 ID (被交换的满箱)',

  FOREIGN KEY (task_id) REFERENCES hybrid_inbound_task(task_id),
  FOREIGN KEY (exchange_task_id) REFERENCES exchange_task(exchange_task_id),
  FOREIGN KEY (rack_id) REFERENCES Racks(rack_id),
  FOREIGN KEY (bin_id) REFERENCES Bins(bin_id),
  FOREIGN KEY (source_bin_id) REFERENCES Bins(bin_id),
  INDEX idx_allocation_status (status),
  INDEX idx_allocation_bin (bin_id, status),
  INDEX idx_allocation_locked (locked_until)
) COMMENT='空箱分配记录表';
```
