...
| ----------------------------------------- | -------------------------------------------------------- | ---------------: | ---------------------: |
| 架构/技术负责人 (Architect/Tech Lead)     | 架构落地、关键机制（幂等/Outbox/状态机）、技术决策与评审 |                1 |                      1 |
| 后端核心 (Backend Core)                   | 编排/状态机、策略框架、数据模型、核心 API                |                2 |                   3–4 |
| 集成工程师 (Integration Engineer)         | WMS/SAP/QMS/SFC 接口对接、契约/Mock、联调闭环            |                1 |                      2 |
| 设备/适配器工程师 (Adapter/Commissioning) | ECS/RCS 适配器、回调幂等、心跳/离线、现场联调            |                1 |                   2–3 |
| 前端 (Frontend: PDA/Console)              | PDA 作业端 + 监控/配置台（PWA/SPA）                      |                1 |                      2 |
| QA/测试 (QA)                              | 自动化回归、端到端场景、UAT 支撑与验收脚本               |                1 |                      2 |
| DevOps/SRE                                | Docker/Compose、监控告警、日志追踪、发布/回滚、演练      |              0.5 |                      1 |
| 产品/项目 (PM/PO)                         | 范围/里程碑、跨团队协同、验收组织与推进                  |              0.5 |                      1 |
